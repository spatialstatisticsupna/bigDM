...
When fitting both the disjoint and k-order neighbour models with the __bigDM__ package (see Orozco et al. [-@orozco2020] and the vignette [bigDM: fitting spatial models](https://emi-sstcdapp.unavarra.es/bigDM/bigDM-1-fitting-spatial-models.html) for more details) parallel or distributed computation strategies can be performed to speed up computations by using the __future__ package [@bengtsson2020unifying]. 

Different computation strategies can be specified through the `plan` argument of the `CAR_INLA()` function (available for __bigDM__ version 0.3 and higher). For further details about these strategies see the [documentation of the __future__ package](https://cran.r-project.org/web/packages/future/index.html)  and its corresponding vignettes.
...
In what follows, we show how to fit the _Disjoint model_ for the Spanish colorectal cancer mortality data using different computation strategies.
