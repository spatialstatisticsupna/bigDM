...
__bigDM__ is a package that implements several (scalable) spatial and spatio-temporal Poisson mixed models for high-dimensional areal count data, with inference in a fully Bayesian setting using the integrated nested Laplace approximation (INLA) technique [@rue2009approximate]. This vignette focuses on how to use the __bigDM__ package to fit the __scalable spatial model's__ proposals described in [@orozco2020] using Spanish colorectal cancer mortality data.
...
# Install bigDM (version 0.4.0) from GitHub repositoy
