...
The disjointed partition is carried out taking as a reference the autonomous regions of Spain. However, a random partition is possible and is explained below.
...
```{r Q matrix, message=FALSE, results=FALSE}
...
The hyperparameter prior assumed `sdunif` and `lunif`, but, you can select others using the `CAR_INLA` function explained below.
...
```{r Fit models disjoint partitions, message=FALSE, results=FALSE}
#### Fit of the models ####
library(INLA)
